...
class HourglassManager {
   ...
   */
  async finishCurrentChunkWithFullContent(endTime, nextUserInput = null, fullSessionContent = null) {
    ...
    const completeAIResponse = fullSessionContent || chunk.aiResponse || chunk.fullSessionContent || '[NO_CONTENT_CAPTURED]';
    ...
    this.log(chalk.green(`   ✅ Complete session content captured (${completeAIResponse.length} chars)`));
  ...
  async saveConversationJSON(chunk) {
    ...
    const jsonFile = path.join(this.projectRoot, '.conversations', `${chunk.chunkId}.json`);
