...
class IntelligentConversationParser {
  ...
  async processConversation(conversationData, options = {}) {
    try {
      ...
      // Log platform detection with appropriate level
      if (aiPlatform === 'unknown') {
        console.log(`⚠️ AI platform detection failed - using generic analysis`);
      } else {
      ...
      // Extract chunk ID
      const chunkId =
        conversationData.metadata?.chunkId ||
  ...
  async processFromSQLite(conversationId, options = {}) {
    try {
      ...
      console.log(
        `✅ ${this.name} SQLite processing completed - wrote rich content to ${routingResults.length} files`
      );
  ...
  extractContent(conversationData) {
    ...
    if (this.verbose) {
      ...
      if (conversationData.messages && conversationData.messages.length > 0) {
        console.log(
          `   First message content: ${conversationData.messages[0].content?.substring(0, 100)}...`
        );
  ...
  extractRealUserIntents(userMessages) {
    ...
    for (const msg of userMessages.slice(-20)) {
      // Look at more messages to find clean ones
  ...
  extractTechnicalWork(aiMessages) {
    ...
    for (const msg of aiMessages) {
      if (
        ...
        msg.content.includes('architecture')
      ) {
        technicalWork.push({
          timestamp: msg.timestamp,
  ...
  analyzeAugmentConversation(richConversation, messages) {
    // First, create a comprehensive conversation summary from ALL messages
    ...
    const agentActions = messages.filter(
      (m) => m.type === 'AI_ACTION' || m.source === 'augment-leveldb'
    );
    ...
    return {
      ...
      flow: this.generateAugmentFlow(messages, conversationSummary),
      cleanupWork: [],
      fileModifications: fileOperations,
      // Augment-specific state tracking
      workingOn: this.extractAugmentWorkingState(messages, conversationSummary),
      blockers: this.extractAugmentBlockers(messages, conversationSummary),
      nextAction: this.extractAugmentNextAction(messages, conversationSummary),
      platform: 'augment',
      // NEW: Include full conversation data for memory files
      fullConversation: conversationSummary.fullConversation,
      conversationMetrics: conversationSummary.metrics,
    };
  }
  ...
  analyzeChatGPTConversation(richConversation, messages) {
    ...
    return {
      userIntents: [
        {
          intent: 'ChatGPT conversation (encrypted)',
      ...
      aiActions: [
        {
          ...
          timestamp: new Date().toISOString(),
          details: 'Encrypted ChatGPT data processed',
        },
      ],
      technicalWork: [],
      decisions: [],
      insights: [
        {
          insight: `ChatGPT conversation processed with ${messages.length} encrypted messages`,
          timestamp: new Date().toISOString(),
          type: 'METADATA',
        },
      ],
  ...
  analyzeGenericConversation(richConversation, messages) {
    ...
    const aiMessages = messages.filter(
      (m) => m.role === 'assistant' || m.type?.includes('AI') || m.type?.includes('ASSISTANT')
    );
  ...
  generateRichContent(classification, analysis, richConversation) {
    ...
    switch (classification) {
      case 'conversation_flow':
        return this.generateRichConversationMemoryContent(
          analysis,
          timestamp,
          conversationId,
          richConversation
  ...
  generateRichConversationMemoryContent(analysis, timestamp, conversationId, richConversation) {
    ...
    const recentQueries = analysis.userIntents
      .slice(-5)
      .map((ui) => `${ui.timestamp}: ${ui.intent}`)
      .join('\n  ');
    const recentActions = analysis.aiActions
      .slice(-5)
      .map((action) => `${action.timestamp}: ${action.type} - ${action.details}`)
      .join('\n  ');
...
source_type=${
      analysis.platform === 'warp'
        ? 'terminal_ai'
        : analysis.platform === 'augment'
          ? 'vscode_agent'
          : 'unknown'
    }
...
key_topics=${analysis.insights
      .map((i) => i.insight.substring(0, 50))
      .join('|')
      .substring(0, 200)}
...
`;
  ...
  generateRichTechnicalContent(analysis, timestamp, conversationId) {
    const technicalWork = analysis.technicalWork
      .map((work) => work.work.substring(0, 200))
      .join('|');
    const fileModifications = analysis.fileModifications
      .map((mod) => `${mod.action}:${mod.files.join(',')}`)
      .join('|');
...
`;
  ...
  generateRichDecisionContent(analysis, timestamp, conversationId) {
    const decisions = analysis.decisions
      .map((d) => `${d.timestamp}:${d.decision.substring(0, 150)}`)
      .join('|');
...
`;
  ...
  /**
   * Extract comprehensive conversation summary from ALL Augment messages
   * This aggregates the full conversation instead of truncating individual messages
   */
  extractAugmentConversationSummary(messages) {
    ...
    const userMessages = messages.filter(
      (m) =>
        m.type === 'user' ||
        m.role === 'user' ||
        (m.metadata && m.metadata.messageType === 'user_request')
    );
    ...
    const aiMessages = messages.filter(
      (m) =>
        m.type === 'assistant' ||
        m.role === 'assistant' ||
        (m.metadata && m.metadata.messageType === 'ai_response')
    );
    ...
    const fullConversation = messages
      .map((m, i) => {
        ...
        return `[${i + 1}] ${role.toUpperCase()} (${timestamp}):\n${content}`;
      })
      .join('\n\n---\n\n');
   ...
   * NOW USES FULL CONVERSATION SUMMARY - NO TRUNCATION
   */
  extractAugmentUserIntents(agentActions, conversationSummary = null) {
    const intents = [];

    // PRIORITY 1: Use conversation summary if available (contains full user queries)
    if (conversationSummary && conversationSummary.userQueries) {
      const userQueryMatches = conversationSummary.userQueries.match(
        /\[User \d+\] (.+?)(?=\n\n\[User \d+\]|\n\n$|$)/gs
      );
      if (userQueryMatches && userQueryMatches.length > 0) {
        ...
        return intents; // Return full intents from summary
      }
    }

    // PRIORITY 2: Extract from individual actions (fallback)
    for (const action of agentActions) {
      ...
      if (action.metadata && action.metadata.messageType === 'user_request') {
        ...
        intents.push({
          ...
          intent: content, // ✅ Use full content (not truncated)
   ...
   * NOW USES FULL CONVERSATION SUMMARY - NO TRUNCATION
   */
  extractAugmentAIActions(agentActions, conversationSummary = null) {
    const actions = [];

    // PRIORITY 1: Use conversation summary if available (contains full AI responses)
    if (conversationSummary && conversationSummary.aiResponses) {
      const aiResponseMatches = conversationSummary.aiResponses.match(
        /\[AI \d+\] (.+?)(?=\n\n\[AI \d+\]|\n\n$|$)/gs
      );
        ...
        aiResponseMatches.forEach((response, index) => {
          ...
          if (cleanResponse.length > 0) {
            actions.push({
              ...
              timestamp: new Date().toISOString(),
              details: cleanResponse, // ✅ FULL AI response, not truncated
              source: 'conversation_summary',
              messageIndex: index + 1,
            });
          }
        });
        return actions; // Return full actions from summary
      }
    }
    ...
    for (const action of agentActions) {
      ...
      actions.push({
        ...
        timestamp: action.timestamp,
        details: content, // ✅ Use FULL content (removed truncation)
   ...
   * NOW USES FULL CONVERSATION SUMMARY - NO TRUNCATION
   */
  extractAugmentTechnicalWork(agentActions, conversationSummary = null) {
    const technicalWork = [];

    // PRIORITY 1: Use conversation summary if available (contains full conversation context)
    if (conversationSummary && conversationSummary.fullConversation) {
      // Extract technical patterns from full conversation
      ...
      const lines = conversationSummary.fullConversation.split('\n');
      ...
      lines.forEach((line, index) => {
        ...
        if (technicalKeywords.some((keyword) => lowerLine.includes(keyword))) {
          technicalWork.push({
            timestamp: new Date().toISOString(),
            work: line.trim(), // ✅ FULL line, not truncated
      ...
      if (technicalWork.length > 0) {
        return technicalWork; // Return full technical work from summary
      }
    }
    ...
    for (const action of agentActions) {
      ...
      technicalWork.push({
        timestamp: action.timestamp,
        work: content, // ✅ Use FULL content (removed truncation)
   ...
   * NOW USES FULL CONVERSATION SUMMARY - NO TRUNCATION
   */
  extractAugmentDecisions(agentActions, conversationSummary = null) {
    const decisions = [];

    // PRIORITY 1: Extract decisions from full conversation
    if (conversationSummary && conversationSummary.fullConversation) {
      ...
      lines.forEach((line, index) => {
        ...
        if (decisionKeywords.some((keyword) => lowerLine.includes(keyword))) {
          decisions.push({
            timestamp: new Date().toISOString(),
            decision: line.trim(), // ✅ FULL decision context, not truncated
      ...
      if (decisions.length > 0) {
        return decisions; // Return full decisions from summary
      }
    }
    ...
    for (const action of agentActions) {
      ...
      if (
        ...
        decisions.push({
          timestamp: action.timestamp,
          decision: content, // ✅ Use FULL content (removed truncation)
          type: 'agent_decision',
   ...
   * NOW USES FULL CONVERSATION SUMMARY - NO TRUNCATION
   */
  generateAugmentFlow(agentActions, conversationSummary = null) {
    // PRIORITY 1: Generate flow from full conversation
    if (conversationSummary && conversationSummary.fullConversation) {
      const messages = conversationSummary.fullConversation.split('\n\n---\n\n');
      const flowSteps = messages
        .filter((msg) => msg.trim().length > 0)
        .map((msg, index) => {
          const firstLine = msg.split('\n')[0];
          const role = firstLine.includes('USER')
            ? 'user'
            : firstLine.includes('ASSISTANT')
              ? 'ai'
              : 'unknown';
          const preview = msg.substring(0, 100).replace(/\n/g, ' ').trim();
          return `${role}_message_${index + 1}`;
        });
      return flowSteps.join('|');
    }

    // PRIORITY 2: Generate from individual actions (fallback)
    if (!agentActions || agentActions.length === 0) {
      return 'augment_workflow|no_actions_detected';
    }
    const flowSteps = agentActions.map(
      (action, index) => `agent_action_${index + 1}` // Simplified flow
    );
    return flowSteps.join('|');
  }
   ...
   * NOW USES FULL CONVERSATION SUMMARY - NO TRUNCATION
   */
  extractAugmentWorkingState(agentActions, conversationSummary = null) {
    // PRIORITY 1: Extract from conversation summary
    if (conversationSummary && conversationSummary.userQueries) {
      // Get the last user query as the current work focus
      const lastQuery = conversationSummary.userQueries
        .split('\n\n')
        .filter((q) => q.trim())
        .pop();
      if (lastQuery) {
        return lastQuery.replace(/\[User \d+\] /, '').trim(); // ✅ FULL query, not truncated
      }
    }

    // PRIORITY 2: Extract from actions (fallback)
    if (agentActions.length === 0) return 'No agent actions detected';
    const latestAction = agentActions[agentActions.length - 1];
    return latestAction.content || 'Unknown work'; // ✅ Use FULL content (removed truncation)
  }

  /**
   * Extract Augment-specific blockers
   * NOW USES FULL CONVERSATION SUMMARY
   */
  extractAugmentBlockers(agentActions, conversationSummary = null) {
    // Check conversation for blocker keywords
    if (conversationSummary && conversationSummary.fullConversation) {
      ...
      const lines = conversationSummary.fullConversation.split('\n');
      const blockerLines = lines.filter((line) =>
        blockerKeywords.some((keyword) => line.toLowerCase().includes(keyword))
      );
      if (blockerLines.length > 0) {
        return blockerLines.join(' | '); // ✅ FULL blocker context
      }
    }
   ...
   * NOW USES FULL CONVERSATION SUMMARY
   */
  extractAugmentNextAction(agentActions, conversationSummary = null) {
    // Extract next action from conversation context
    if (conversationSummary && conversationSummary.fullConversation) {
      const nextActionKeywords = ['next', 'todo', 'will', 'should', 'need to', 'plan to'];
      const lines = conversationSummary.fullConversation.split('\n');
      const nextActionLines = lines.filter((line) =>
        nextActionKeywords.some((keyword) => line.toLowerCase().includes(keyword))
      );
      if (nextActionLines.length > 0) {
        return nextActionLines[nextActionLines.length - 1].trim(); // ✅ FULL next action context
      }
    }

    return 'Continue development workflow';
  }
  ...
  generateConversationMemoryContent(analysis, timestamp, chunkId) {
    const sourceHeader = this.currentPlatform
      ? `[${this.currentPlatform.toUpperCase()}]`
      : '[UNKNOWN]';
    // Extract real user intent from analysis
    const realUserIntent =
      analysis.userIntents && analysis.userIntents.length > 0
...
ai_actions=${
      Array.isArray(analysis.aiActions)
        ? analysis.aiActions.map((a) => (typeof a === 'object' ? a.type : a) || 'unknown').join(',')
        : analysis.aiActions || 'processing'
    }
key_topics=${
      Array.isArray(analysis.keyTopics)
        ? analysis.keyTopics.join(',')
        : analysis.keyTopics || 'GENERAL'
    }
...
`;
  ...
  generateTechnicalContent(analysis, timestamp, chunkId) {
    const insights = Array.isArray(analysis.insights)
      ? analysis.insights
          .map((i) => (typeof i === 'object' ? i.insight || i.details || 'insight' : i))
          .join('|')
      : 'no_insights';
    const topics = Array.isArray(analysis.keyTopics) ? analysis.keyTopics.join(',') : 'GENERAL';
...
`;
  ...
  generateDecisionContent(analysis, timestamp, chunkId) {
    const decisions = Array.isArray(analysis.decisions)
      ? analysis.decisions
          .map((d) => (typeof d === 'object' ? d.decision || d.type || 'decision' : d))
          .join('|')
      : 'no_decisions';
...
`;
  ...
  generateWorkStateContent(analysis, timestamp, chunkId) {
    const actions = Array.isArray(analysis.aiActions)
      ? analysis.aiActions
          .map((a) => (typeof a === 'object' ? a.type || a.action || 'action' : a))
          .join(',')
      : 'processing';
...
`;
  ...
  extractWarpWorkingState(aiActions) {
    ...
    return workingStates.length > 0
      ? workingStates[workingStates.length - 1]
      : 'ongoing development';
  }
  ...
  extractWarpDecisions(messages) {
    ...
    for (const msg of messages) {
      ...
      if (
        ...
        lowerContent.includes('chose to') ||
        lowerContent.includes('decided to')
      ) {
  ...
  createStructuredSections(analysis) {
    return {
      ...
      state: {
        ...
        metadata: {
          source: 'platform_aware_analysis',
